...
- 사용자가 현재 이용 중인 서비스에 도구형 에이전트가 포함되어 있다면 그 경로를 먼저 안내합니다. ChatGPT에서는 Codex, Claude에서는 Claude Code, Google 계열 서비스에서는 해당 서비스의 에이전트를 우선합니다.
- 현재 서비스에서 파일 읽기·쓰기, 명령 실행, Anki 연결을 제공하지 않거나 사용자가 다른 선택지를 요청할 때만 다른 제품을 대안으로 소개합니다.
- 화면에서 버튼을 눌러 사용하는 방식이 편하면 로컬 폴더를 프로젝트로 여는 데스크톱 앱을 우선 고려합니다.
- 명령어를 입력하는 방식이 편하면 운영체제를 공식 지원하는 명령줄 도구(CLI)를 고려합니다.
...
이 저장소는 Anki Desktop 안에서 실행되는 네이티브 AnkiMCP 애드온과 `http://127.0.0.1:3141` 연결 한 가지만 안내합니다. 제품별 MCP 설정 화면과 파일 위치는 다를 수 있지만, 다른 Anki 연결 방식으로 바꾸거나 여러 방식을 동시에 구성하지 않습니다. 자세한 절차는 [`anki-mcp-setup.md`](anki-mcp-setup.md)를 따릅니다.
