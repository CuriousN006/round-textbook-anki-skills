...
- 명령어를 입력하는 창에서 필요한 작업을 실행할 수 있는가?
- AI가 Anki 같은 다른 프로그램을 사용할 수 있게 해 주는 연결 기능(MCP 또는 커넥터)이 있는가?
...
먼저 사용자가 현재 어느 챗봇 서비스에서 질문하고 있는지 확인하세요. 그 서비스에 로컬 파일과 명령을 다룰 수 있는 에이전트 기능이 포함되어 있다면 그 경로를 가장 먼저 안내합니다. 예를 들어 ChatGPT에서는 Codex, Claude에서는 Claude Code, Google 계열 서비스에서는 해당 서비스의 에이전트 경로를 우선합니다. 현재 서비스에서 필요한 기능을 제공하지 않거나 사용자가 다른 선택지를 원할 때만 다른 제품을 대안으로 소개하세요.

추가 정보가 필요할 때만 기술 지식을 요구하지 않고 다음을 한 번에 하나씩 물어보세요.

1. Windows, macOS, Linux 중 어떤 운영체제를 사용하는가?
2. 화면에서 버튼을 눌러 사용하는 방식과 명령어를 입력하는 방식 중 어느 쪽이 더 편한가?

그 답에 맞춰 우선 경로를 설명하고 최신 공식 설치 문서를 확인하세요. 저장소에 적힌 오래된 설치 명령을 현재의 절대적인 방법으로 단정하지 마세요.
...
- [Claude Code 공식 설치 안내](https://code.claude.com/docs/en/setup): 명령어 입력 창(터미널)과 지원 편집기에서 로컬 프로젝트를 다룰 수 있습니다.

현재 서비스의 에이전트를 먼저 추천하되, 필요한 기능이 없거나 사용자가 다른 선택지를 원하면 대안을 안내하세요. 로그인 비밀번호나 외부 서비스 연결에 사용하는 비밀 키를 채팅창에 붙여넣도록 요구하지 마세요. 로그인은 각 제품의 공식 로그인 화면이나 컴퓨터의 안전한 로그인 저장 기능을 사용하도록 안내하세요.
...
이 진단은 파일이나 Anki 데이터를 수정하지 않습니다. 운영체제, Python과 필수 패키지, Anki 실행 여부, 그리고 내 컴퓨터 안에서 Anki 연결에 사용하는 번호 `3141`의 응답만 확인합니다.
...
- 프로그램 설치, 필요한 파일 다운로드, 인터넷 접근, 작업 폴더 밖의 쓰기, Anki 연결 설정 변경, Anki 노트 생성·수정·삭제는 실행 전에 이유와 범위를 설명하고 승인을 받습니다.
...
- Anki 연결은 [`docs/anki-mcp-setup.md`](docs/anki-mcp-setup.md)의 네이티브 AnkiMCP 방식만 사용합니다. 여러 연결 방식을 함께 설치하거나 비교하지 않습니다.

## 6. 사용자에게 최소 정보만 요청하세요

다음 정보가 없을 때만 쉬운 말로 한 번에 하나씩 질문하세요.

1. PDF가 이미 들어 있는 기존 폴더의 위치
2. 만들고 싶은 상위 덱 이름
...
이 도구는 PDF 폴더를 새로 만들지 않습니다. 입력한 기존 폴더를 읽고, 확인을 받은 뒤 `source-details.local.md`와 `source-manifest.local.json`만 만듭니다. 이 파일들은 Git에서 무시됩니다. 공개 커밋에 절대 경로, 저작권 PDF 내용, 개인 덱 정보가 들어가지 않았는지 확인하세요.
...
4. 저장된 앞면(Front), 뒷면(Back), 덱, 태그, 이미지, 정답을 Anki에서 다시 읽습니다.
